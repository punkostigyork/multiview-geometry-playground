...
def test_camera_projection():
    # Create a simple Identity Camera
    K = np.array([[1000, 0, 500], [0, 1000, 500], [0, 0, 1]])
    R = np.eye(3)
    t = np.zeros((3, 1))
    cam = PinholeCamera(K, R, t)
    
    # Define a 3D point exactly 2 units in front of the camera
...
def test_ransac_outlier_rejection():
    # Create 20 perfect points matching an identity relationship
    pts1 = np.random.rand(20, 2) * 100
    pts2 = pts1.copy()
    pts2[:, 0] += 50  # Shifted by 50 pixels
    
    # Add 5 "Zombies" (total garbage outliers)
